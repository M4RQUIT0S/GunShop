...
- **Aplicarlas contra un Postgres.** `db/schema.sql` sí está probado: se
  aplica, se llena y pasa `db/smoke.sql`. Estas migraciones **no** se han
  ejecutado todavía contra una base real, sólo revisadas con
  `node db/supabase/revisa.js`. Antes de fiarse, pasarlas por un proyecto de
  prueba de Supabase y correr el equivalente de `smoke.sql`.
