...
- [ ] 1. Reglas de regimen en `js/catalog.js` (que exige cada familia, calibre
      de la municion, cartuchos por caja y tope TCCM). Una sola fuente para la
      cesta y para el seed de la base.
- [ ] 2. `js/cart.js` + panel: lineas, cantidades, total, avisos de regimen,
      persistencia. La ficha deja de sumar a un contador y suma a la cesta.
- [ ] 3. `js/search.js` + panel: busca sobre el catalogo ya construido y
      manda el resultado a la rejilla.
- [ ] 4. `js/account.js` + panel: perfil local (CLU, vencimiento, TCCM). La
